...
def question18(question17_win):
    def q18_correct():
        correct()
        results(question18_win)

    def q18_incorrect():
        messagebox.showinfo('Result','Incorrect, The answer is the Voith-Arena')
        results(question18_win)

    question18_win = Toplevel()
    question18_win.geometry('+0+0')
    question18_win.resizable(False,False)
    question18_win.title('Bundesliga 25-26 Stadiums Quiz - Q18')

    q18_label = tk.Label(
        question18_win,
        text='What is the Heidenheim Stadium called?',
        font=('Arial',20)
    )
    q18_label.grid(
        row=1,
